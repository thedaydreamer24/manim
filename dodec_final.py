...
class dodec_final(MovingCameraScene):

    
    def construct(self):
        M = [0.0681483474219, 3.3460652149512, 0]
        dodecagon = RegularPolygon(n=12, radius = 3.8637033051563, color = WHITE, stroke_width = 4).move_to(M)
        ...
        #position for the 
        W = [-12.7962786532271, 3.3460652149512, 0]
        ...
        circle = Circle(radius = 3.8637033051563, color = WHITE, stroke_width = 4).move_to(M)
        
        self.play(GrowFromCenter(dodecagon, point_color = "#0A47C2"))
        
        
        
        self.play(Create(circle), FadeIn(line, text))
        self.wait(2)
        ...
        d_copy_t = dodecagon.copy()
        d_copy_t.move_to(W).shift(3*LEFT)
        self.play(FadeIn(d_copy_t))
        self.play(self.camera.frame.animate.move_to(d_copy_t).shift(2*DOWN))
        self.wait(2)
        
        l1 = Line(d_copy_t.get_vertices()[3], d_copy_t.get_vertices()[9])
        l2 = Line(d_copy_t.get_vertices()[2], d_copy_t.get_vertices()[8])
        l3 = Line(d_copy_t.get_vertices()[1], d_copy_t.get_vertices()[7])
        l4 = Line(d_copy_t.get_vertices()[0], d_copy_t.get_vertices()[6])
        l5 = Line(d_copy_t.get_vertices()[11], d_copy_t.get_vertices()[5])
        l6 = Line(d_copy_t.get_vertices()[10], d_copy_t.get_vertices()[4])
        ...
        poly = Polygon(d_copy_t.get_vertices()[7], 
                       d_copy_t.get_vertices()[8], 
                       d_copy_t.get_center(), 
                       color = WHITE, stroke_width = 4, fill_color = "#0A47C2", fill_opacity=1)
        poly.joint_type=LineJointType.ROUND 
        self.play(FadeIn(poly))
        
        
        eq = MathTex("A(", "\phantom{xxx}", ")\ = \  ").scale(2).set_color(WHITE).set_stroke(width = 1).next_to(d_copy_t, DOWN, buff=2).shift(0.3*LEFT)
        d_c = dodecagon.copy()
        d_c.scale(0.175).move_to(eq).shift(0.71*LEFT).shift(0.4*RIGHT)
        eq2 = VGroup(eq, d_c).shift(2.4*LEFT).scale(0.8)
        
        self.play(FadeIn(eq2))
        twlv = MathTex("12").next_to(eq2, buff = 0.6).scale(2)
        mult = MathTex("\\times").next_to(twlv, buff = 0.6).scale(2)
        
        eqc = MathTex("A(", "\phantom{xxx}", ")\  \  ").scale(2).set_color(WHITE).set_stroke(width = 1).next_to(d_copy_t, DOWN, buff=2)
        eqc.scale(0.8)
        eqc.next_to(mult, buff = 0.5)
         
        self.play(FadeIn(twlv, mult))
        #self.play(FadeIn())
        poly_copy = poly.copy()
        self.play(FadeIn(eqc), poly_copy.animate.move_to(eqc).scale(0.225).shift(0.325*RIGHT))
        self.wait(2)
        
        
        #GOING BACK TO ORIGINAL
        self.play(self.camera.frame.animate.move_to(dodecagon).shift(2*DOWN), 
                  FadeOut(d_copy_t, poly_copy, lines_g, poly, eq2, twlv, mult, eqc))
        self.wait()
        ...
        l_1 = Line(dodecagon.get_vertices()[3], dodecagon.get_vertices()[4], color = PURE_RED, stroke_width = 5).set_z_index(32)
        l_2 = Line(dodecagon.get_vertices()[4], dodecagon.get_vertices()[5], color = PURE_RED, stroke_width = 5).set_z_index(32)
        l_3 = Line(dodecagon.get_vertices()[5], dodecagon.get_vertices()[6], color = PURE_RED, stroke_width = 5).set_z_index(32)
        
        self.play(FadeIn(l_1,l_2,l_3))
        
        TI = Line(dodecagon.get_vertices()[5], T)
        TJ = Line(dodecagon.get_vertices()[6], T)
        UH = Line(dodecagon.get_vertices()[4], U)
        UI = Line(dodecagon.get_vertices()[5], U)
        VH = Line(dodecagon.get_vertices()[4], V)
        VG = Line(dodecagon.get_vertices()[3], V)
        ...
        MG = Line(dodecagon.get_vertices()[3], M)
        MV = Line(V, M)
        MH = Line(dodecagon.get_vertices()[4], M)
        MU = Line(U, M)
        MI = Line(dodecagon.get_vertices()[5], M)
        MT = Line(T, M)
        MJ = Line(dodecagon.get_vertices()[6], M)
        ...
        p10 = Polygon(dodecagon.get_vertices()[9],
                      ...
                      fill_color = "#0A47C2", 
                      fill_opacity=1, 
                      stroke_width = 4, stroke_color = WHITE).set_z_index(3)
        ...
        self.play(FadeIn(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10), FadeOut(l_1,l_2,l_3))
        ...
        self.wait(0.5)
        self.play(Indicate(r, color = RED, scale_factor=1.3 ), Indicate(line_copy, color = RED, scale_factor=1), run_time=1.5)
        ...
        square = Square(side_length = 2*3.8637033051563, fill_opacity=1, color = WHITE, stroke_width = 4, fill_color = "#D68000").set_z_index(2).move_to(M).set_z_index(1)
        self.play(FadeOut(dodecagon_copy, fbox, feq, r, r_copy, circle),  self.camera.frame.animate.shift(2*UP), 
                  FadeIn(square, text), 
