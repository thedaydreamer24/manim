...
class tri(MovingCameraScene):
    ...
    def construct(self):
        ...
        #Zoom out
        X = self.camera.frame.scale(2).move_to(dodecagon)
        self.add(X)
        ...
        distance_vector = p1_pos.get_center() - p1.get_center()

        # Updater function for fill_arc2
        def update_p1(mob, dt):
            #angle = 4*PI/2 - PI / 2 * t.get_value()
            mob.become(
                Polygon(I, J, T, fill_color = "#005E2C", fill_opacity = 1, stroke_color = WHITE).set_z_index(5).shift(distance_vector * t.get_value())
            )
        ...
        # Calculate the distance vector from initial position to the final position beside s6_copy
        distance_vector = p2_pos.get_center() - p2.get_center()

        # Updater function for fill_arc2
        def update_p2(mob, dt):
            #angle = 4*PI/2 - PI / 2 * t.get_value()
            mob.become(
                Polygon(I, U, H, fill_color = "#005E2C", fill_opacity = 1, stroke_color = WHITE).set_z_index(5).shift(distance_vector * u.get_value())
            )
        ...
        # Calculate the distance vector from initial position to the final position beside s6_copy
        distance_vector = p3_pos.get_center() - p3.get_center()

        # Updater function for fill_arc2
        def update_p3(mob, dt):
            #angle = 4*PI/2 - PI / 2 * t.get_value()
            mob.become(
                Polygon(H, V, G, fill_color = "#005E2C", fill_opacity = 1, stroke_color = WHITE).set_z_index(5).shift(distance_vector * v.get_value())
            )

        p3.add_updater(update_p3)

        # Play the animation
        self.play(v.animate.set_value(1), run_time=2)
        ...
        # Moving p9
        self.play(Rotate(p9, angle = PI/2, about_point = G))
        #self.wait()
        ...
        #Moving p8
        self.play(Rotate(p8, angle = PI/3, about_point = M))
        self.play(p8.animate.move_to(p5_pos))
        #self.wait()
        
        #Moving p6
        self.play(Rotate(p6, angle = PI/1.5, about_point = M))
        self.play(p6.animate.move_to(p7_pos))
        #self.wait()
        
        #Moving p5
        self.play(Rotate(p5, angle = -PI/3, about_point = M))
        self.play(p5.animate.move_to(p8_pos))
        self.wait(4)
        ...
        rsq = MathTex("r^2").next_to(mul, buff = 0.6).scale(1.9).shift(0.1*UP)
        ...
        fbox = SurroundingRectangle(feq, color="#0A47C2", buff=0.5, corner_radius=0.1, stroke_width=6)
