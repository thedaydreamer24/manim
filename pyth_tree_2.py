import argparse
import math
from math import atan, atan2, ceil, pi, sqrt
...
class test2(MovingCameraScene):
    ...
    def construct(self):
        ...
        right_triangle.joint_type=LineJointType.ROUND 
        

        #self.add(give_poly(r.get_value()))
        Q = [0.5,0.5,0]
        self.add(self.camera.frame.move_to(Q))
        ...
        # l = always_redraw(
        #     lambda:
        #     Line(new_tree[1].get_vertices()[1],new_tree[0].get_vertices()[2])
        # )

        ninety = always_redraw(
            lambda:
            Square(side_length=0.16,
                        stroke_width = 0,
                        fill_color = "#C22300",
                        fill_opacity=1).shift(new_tree[1].get_vertices()[1]-0.16/2*UL).set_z_index(-1).rotate(-atan(1/r.get_value()),about_point=new_tree[1].get_vertices()[1])
        )
        # ninety.move_to(l.get_start()).rotate(PI/4).rotate(PI/2, about_point=UP/2+RIGHT/2).shift(0.03*DOWN).set_z_index(6)
        self.add(ninety, new_tree, right_triangle,txt)
        # self.wait()
        # self.play(FadeIn(txt))
        self.play(r.animate.set_value(0.65),run_time=2.5)
        self.wait()
        self.play(r.animate.set_value(1/0.65),run_time=5)
        self.wait()
        self.play(r.animate.set_value(1),run_time=2.5)
        self.wait(3)
        # self.play(FadeOut(ninety, new_tree, txt, right_triangle))
        self.wait(3)
...
class test4(MovingCameraScene):
    def construct(self):
        ...
        self.wait()
        p = Polygon(*[ORIGIN,RIGHT,UP/2+RIGHT/2])
        self.add(p)
        self.wait()
